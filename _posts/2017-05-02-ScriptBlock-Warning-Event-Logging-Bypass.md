...
The PowerShell wizard, Daniel Bohannon ([@danielhbohannon](https://twitter.com/danielhbohannon)), came through with a solution to the previously mentioned problem with scripting this bypass. We can simply encode our desired payload using Base64 and then decode and invoke it during execution. Originally I didn't believe this would work since encoding techniques, such as Base64, is unraveled prior to hitting ScriptBlock logs and would be scanned just the same. But we are okay with our payload hitting ScriptBlocks logs, *as long as it is post-bypass*. And if we encode the payload, the bypass will be executed just **before** that encoding is unraveled and our payload is scanned for suspicious scripts. This allows us to bypass the Warning-level logs within any given PowerShell script, so long as it appears encoded in the script and it is decoded/invoked during execution. Thanks Daniel!
